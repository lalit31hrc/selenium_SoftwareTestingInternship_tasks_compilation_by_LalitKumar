...
public class AmazonProductDetailsTest extends TestBase{
	...
	public void VerifyProductDetailsAndAddToCart() throws InterruptedException {
	    ...
	    Assert.assertFalse(actualTitle.isEmpty(), "Product title should not be empty");
	    Assert.assertTrue(actualTitle.toLowerCase().contains("watch"), "Product title should contain 'watch'");
	    Assert.assertFalse(actualPrice.isEmpty(), "Product price should not be empty");
